...
func (c *Client) GenerateCSR(ctx context.Context, subject device.CertSubject) (string, error) {
	...
	// subjectAltName is gated on the worker side: for PrivateKeyInstaller devices
	// (which AudioCodes is), the worker never populates subject.SANs because the
	// firmware CSR API doesn't support subjectAltName (returns 400 "JSON form syntax
	// error"). SANs are instead included in the external CSR generated by the connector
	// for the ACME flow. For non-PrivateKeyInstaller devices that do support SANs,
	// the worker populates subject.SANs when IncludeHostAsSAN is set.
